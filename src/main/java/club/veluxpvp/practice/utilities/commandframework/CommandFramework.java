package club.veluxpvp.practice.utilities.commandframework;
	...
	public boolean handleCommand(CommandSender sender, org.bukkit.command.Command cmd, String label, String[] args) {
		for (int i = args.length; i >= 0; i--) {
			...
			if (commandMap.containsKey(cmdLabel)) {
				...
				if (command.partyOnly() && sender instanceof Player) {
					Party party = Practice.getInstance().getPartyManager().getPlayerParty((Player) sender);
					
					if(party == null) {
						sender.sendMessage(ChatUtil.TRANSLATE("&cYou are not in a party!"));
						return true;
					}
				}
				try {
