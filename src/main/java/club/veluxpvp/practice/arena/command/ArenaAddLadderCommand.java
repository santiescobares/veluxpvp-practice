...
public class ArenaAddLadderCommand extends ArenaCommand {

	@Command(name = "arena.addladder")
	public void execute(CommandArgs cmd) {
		...
		if(args.length >= 2) {
			...
			if(!arena.addSupportedLadder(ladder)) {
				player.sendMessage(ChatUtil.TRANSLATE("&cThis arena already contains that ladder!"));
				return;
			}
			
			player.sendMessage(ChatUtil.TRANSLATE("Ladder &b" + ladder.name + " &aadded &fto arena &b" + arena.getName() + "&f!"));
		} else {
			player.sendMessage(ChatUtil.TRANSLATE("&cUsage: /arena addladder <arenaName> <ladder>"));
		}
	}
}
